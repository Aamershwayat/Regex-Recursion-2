...
function capitalA(s) {
    ...
    var upperCase = string.match(/^[A-Z]*$/)

    var lowerCase = string.match(/^[a-z]*$/)

    var numbers = string.match(/ \\d/)

    var symbols = string.match(/ [-+_!@#$%^&*., ?]/)

    var capitalA = string.endsWith('A')


    if (upperCase && lowerCase && capitalA== true != null) {
        theAnswer= true
    } else {
        theAnswer= false
    }
...
function ioEmail(email) {
    // Add your logic.
        let endOfemail = email.match(/^[\w]+@([\w-]+\.)+[\w-]{2,4}$/g);
        if(endOfemail != null){
            return true;
        }else{
            return false;
